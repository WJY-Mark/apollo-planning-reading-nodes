...
bool Box2d::HasOverlap(const Box2d &box) const {//超平面分割定理：若两个凸多边形无重合，一定可以找到一条沿着边方向的直线，将其分开。
  if (box.max_x() < min_x() || box.min_x() > max_x() || box.max_y() < min_y() ||
      box.min_y() > max_y()) {//现在全局XY方向上投影，粗略判断是否重合  若没有重合则返回false
    return false;
  }
  //下面将把这两个box投影到四个方向，分别为box1的length，width方向和box2的length，width方向 只要能在一个方向上分开，则不存在重合。
//这段程序要用向量的思想 ab=|a||b|cos(θ)若a为单位向量，则得到的是b在a方向的投影
  //box定义：正常右手坐标系，x轴右，y轴上 为正，角度逆时针为正，与角度方向同向的为length边，垂直的是width边
  const double shift_x = box.center_x() - center_.x();
  const double shift_y = box.center_y() - center_.y();

  const double dx1 = cos_heading_ * half_length_;  //box1中心画个十字，(dx1,dy1)为十字与width边的交点与center连线的向量
  const double dy1 = sin_heading_ * half_length_;  
  const double dx2 = sin_heading_ * half_width_;   //(dx2,dy2)为十字与length边的交点与center连线的向量
  const double dy2 = -cos_heading_ * half_width_;
  const double dx3 = box.cos_heading() * box.half_length(); //同上，只是变成了box2
  ...
  return std::abs(shift_x * cos_heading_ + shift_y * sin_heading_) <=
             std::abs(dx3 * cos_heading_ + dy3 * sin_heading_) +
                 ...
                 half_length_ &&  //把box1中心到box2中心的向量 和 box2的半长，半宽向量 投影在box1的heading方向 在一维判断是否有重合
         std::abs(shift_x * sin_heading_ - shift_y * cos_heading_) <=
             std::abs(dx3 * sin_heading_ - dy3 * cos_heading_) +
                 std::abs(dx4 * sin_heading_ - dy4 * cos_heading_) +
                 half_width_ &&  //把box1中心到box2中心的向量 和 box2的半长，半宽向量 投影在box1的width方向(与heading垂直) 在一维判断是否有重合
         std::abs(shift_x * box.cos_heading() + shift_y * box.sin_heading()) <=
             std::abs(dx1 * box.cos_heading() + dy1 * box.sin_heading()) +
                 std::abs(dx2 * box.cos_heading() + dy2 * box.sin_heading()) +
                 box.half_length() && //把box1中心到box2中心的向量 和 box1的半长，半宽向量 投影在box2的width方向(与heading垂直) 在一维判断是否有重合
         std::abs(shift_x * box.sin_heading() - shift_y * box.cos_heading()) <=
             std::abs(dx1 * box.sin_heading() - dy1 * box.cos_heading()) +
                 std::abs(dx2 * box.sin_heading() - dy2 * box.cos_heading()) +
                 box.half_width(); //把box1中心到box2中心的向量 和 box1的半长，半宽向量 投影在box2的width方向(与heading垂直) 在一维判断是否有重合
}
