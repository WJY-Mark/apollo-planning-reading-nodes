...
Status StBoundaryMapper::MapWithoutDecision(PathObstacle *path_obstacle) const
{
  ...
  {
    boundary.SetBoundaryType(prev_st_boundary.boundary_type()); //这reference_line_stboundary和prev_st_boundary啥意思？？
  }
...
{ //ST图平行四边形的下边界点
  ...
  { //静态障碍物  轨迹点数量为0
    ...
    { //遍历sl坐标系下规划完的路径点
      ...
      { //检测与本车box是否有重叠 CheckOverlap函数实际也是调用box2d类判断重合的类方法
    ...
    PathObstacle *path_obstacle, const ObjectDecisionType &decision) const
{
  ...
  if (decision.has_follow())
  {
    ...
    b_type = StBoundary::BoundaryType::FOLLOW; //对boundary设置类型：跟车
  }
  else if (decision.has_yield())
  {
    ...
    b_type = StBoundary::BoundaryType::YIELD; //对boundary设置类型：领车
  }
  else if (decision.has_overtake())
  {
    characteristic_length = std::fabs(decision.overtake().distance_s());
    b_type = StBoundary::BoundaryType::OVERTAKE; //对boundary设置类型：超车
  }
  else
  {
  ...
  boundary.SetBoundaryType(b_type); //对boundary设置类型：超车、跟车等
  boundary.SetId(path_obstacle->obstacle()->Id());
  boundary.SetCharacteristicLength(characteristic_length);
  path_obstacle->SetStBoundary(boundary); //最后把boundary放到path_obstacle的类成员中
